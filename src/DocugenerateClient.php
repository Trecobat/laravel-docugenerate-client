...
class DocugenerateClient
{
    ...
    {
        $this->apiKey = env( "DOCUGENERATE_KEY","" );
        if( $templateId != null ){
    ...
    public function download($name = null){
        ...
        Log::debug("Ecriture du document généré en local sous " . '/tmp/'.$doc->getFilename());
    ...
    /**
     * Génération et téléchargement d'un document
     * Le document est enregistré dans le path en paramètre
     *
     * @param $path
     * @param null $name
     * @return bool
     */
    public function downloadInPath( $path, $name = null){
        $doc = $this->generate($name);
        Log::debug("Ecriture du document généré en local sous " . $path);
        Storage::disk('local')->put($path, file_get_contents($doc->getDocUri()));
        return $path;
    }
